...
fit_cox_model <- function(data, stratum, covariates) {
  ...
  models <- map(
    ...
    .f = ~ {
      ...
      formula <- as.formula(paste("Surv(follow_up_time, death_all) ~", covariates))
    
      coxph(formula, data = filtered_data)
  })
...
calculate_follow_up_time <- function(dataframes, covariates) {
